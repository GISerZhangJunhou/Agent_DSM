...
Example:
    python models/RFK_V164_from_fused_csv.py ^
      --csv "E:/Agent_DSM/your_training.csv" ^
      --out "E:/Agent_DSM/RFK_V164_OUT" ^
...
from services import rfk_clean_v164  # noqa: E402
...
class FusedCsvOwner:
    def __init__(self, csv_path: Path, out_dir: Path, raster_root: Path | None, target_col: str | None, x_col: str | None, y_col: str | None, lon_col: str | None, lat_col: str | None):
        ...
        self.task_id = None
        self.run_mode = "v164_fused_csv_standalone"
    ...
    def _krige_residuals(self, state: dict[str, Any], df: pd.DataFrame, target: dict[str, Any] | None):
        x, y, _ = rfk_clean_v164._make_xy(self, df, target)
...
def main():
    ...
    # Feature detection is handled by rfk_clean_v164.
    target = {"region": "fused_csv", "target_crs": "EPSG:32648", "fused_csv_source": str(csv_path)}
    report, pred = rfk_clean_v164.run_clean_rfk_model(owner, pd.DataFrame(), [], out_dir, target, [])
    print("V164 RFK 完成")
