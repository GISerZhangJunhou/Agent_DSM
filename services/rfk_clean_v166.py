...
def _stable_hash_text(text: str) -> str:
    """Deterministic backend hash for internal integrity checks.

    User-facing run codes are six-digit random numbers, but metric/source
    integrity must use stable hashes so the backend can detect whether the
    current matrix, features and rasters really changed.
    """
    return hashlib.sha256(str(text or "").encode("utf-8", errors="ignore")).hexdigest()


def _runtime_metric_fingerprint(*, model_ready_csv: str | None, features: list[str], map_scope: str, raster_map: dict | None, row_count: int, target: dict | None = None) -> str:
    """Fingerprint the exact current run inputs that determine user-facing metrics.

    This is backend-only and must be deterministic. It proves metrics were
    recalculated for the current model-ready CSV, current deployable feature set,
    current raster support and current output scope.
    """
    payload: dict[str, Any] = {
        "features": list(features or []),
        "map_scope": str(map_scope or ""),
    ...
    try:
        p = Path(str(model_ready_csv or ""))
        if p.exists():
            st = p.stat()
            payload["model_ready_csv"] = {"path": str(p), "size": int(st.st_size), "mtime_ns": int(st.st_mtime_ns)}
            h = hashlib.sha256()
            with p.open("rb") as f:
                for chunk in iter(lambda: f.read(1024 * 1024), b""):
                    h.update(chunk)
            payload["model_ready_csv_sha256"] = h.hexdigest()
        else:
            payload["model_ready_csv"] = {"path": str(model_ready_csv or ""), "missing": True}
    except Exception as exc:
        payload["model_ready_csv_hash_error"] = str(exc)
    try:
        for k, v in sorted((raster_map or {}).items(), key=lambda kv: str(kv[0])):
            pp = Path(str(v))
            if pp.exists():
                st = pp.stat()
                payload["raster_map"][str(k)] = {"path": str(pp), "size": int(st.st_size), "mtime_ns": int(st.st_mtime_ns)}
            else:
                payload["raster_map"][str(k)] = {"path": str(v), "missing": True}
    except Exception as exc:
        payload["raster_map_error"] = str(exc)
...
def _build_mappable_feature_plan(owner: Any, csv_features: list[str], target: dict[str, Any]) -> dict[str, Any]:
    """V166: separate training-diagnostic features from deployable map features.
    ...
    target["v166_feature_deployment_plan"] = plan
    target["v166_feature_raster_map"] = alias_map
    if clcd_raster:
        target["v166_clcd_mask_raster"] = clcd_raster
...
def _prepare_training_matrix_forced(df: pd.DataFrame, feature_cols: list[str], target_col: str) -> tuple[pd.DataFrame, np.ndarray, list[str], dict[str, Any]]:
    ...
    if not feature_cols:
        raise RuntimeError("V166 未检测到有预测栅格支撑的协变量列，无法输出正式GeoTIFF。")
    ...
    kept=[]; feature_audit=[]
    hard_drop_missing = float(os.getenv("PRO_V166_HARD_DROP_FEATURE_MISSING_RATE", os.getenv("PRO_V165_HARD_DROP_FEATURE_MISSING_RATE", os.getenv("PRO_V164_HARD_DROP_FEATURE_MISSING_RATE", "0.98"))))
    ...
    if not kept:
        raise RuntimeError("V166 所有可出图协变量均为全缺失/近全缺失或常量，无法输出正式GeoTIFF。")
    X = X_raw[kept].copy()
    impute_values={}
    for c in kept:
    ...
    audit={
        ...
        "v166_feature_policy": "forced deployable feature set: only fused-CSV columns with matched prediction rasters",
    }
...
def _candidate_space(n: int, p: int) -> list[dict[str, Any]]:
    ...
    """
    rf_candidates = []
    n_estimators_options = [int(x) for x in (os.getenv("PRO_CLEAN_RFK_TREE_OPTIONS", "500,800") or "500,800").replace(";", ",").split(",") if str(x).strip().isdigit()] or [500, 800]
    ...
    max_candidates = int(os.getenv("PRO_CLEAN_RFK_CANDIDATE_COUNT", os.getenv("PRO_V164_CANDIDATE_COUNT", "16")))
...
def _write_formal_gcp_oof(owner: Any, out_dir: Path, X: np.ndarray, y: np.ndarray, df: pd.DataFrame, groups: np.ndarray | None, target: dict[str, Any], rf_params: dict[str, Any], rfk_params: dict[str, Any], feature_cols: list[str], label: str = "mappable") -> str:
    """Write a strict OOF calibration table for formal GCP + AOA execution.

    The table is generated from repeated spatial GroupShuffleSplit predictions so
    the uncertainty module consumes real holdout residuals rather than fitted
    residuals or placeholder values.  Repeated splits may test the same sample
    more than once; values are aggregated by sample_index before writing.
    """
    try:
        if GroupShuffleSplit is None or X is None or y is None or df is None or len(y) < 20:
            return ""
        n_splits = int(os.getenv("PRO_GCP_OOF_SPATIAL_SPLITS", os.getenv("PRO_V164_REPEATED_SPATIAL_SPLITS", "15")))
        n_splits = max(1, n_splits)
        test_size = float(os.getenv("PRO_GCP_OOF_TEST_SIZE", os.getenv("PRO_CLEAN_RFK_HOLDOUT_TEST_SIZE", "0.30")))
        if groups is None or len(np.unique(groups)) < 3:
            splitter = ShuffleSplit(n_splits=n_splits, test_size=test_size, random_state=20260629)
            splits = splitter.split(X, y)
            split_name = "Repeated-ShuffleSplit-random"
        else:
            splitter = GroupShuffleSplit(n_splits=n_splits, test_size=test_size, random_state=20260629)
            splits = splitter.split(X, y, groups=groups)
            split_name = "Repeated-GroupShuffleSplit-spatial"

        try:
            xcoord, ycoord, coord_crs = _make_xy(owner, df, target)
        except Exception:
            xcoord = np.arange(len(df), dtype="float64")
            ycoord = np.zeros(len(df), dtype="float64")
            coord_crs = str((target or {}).get("target_crs") or "EPSG:32648")
        ...
        for fold, (train_idx, test_idx) in enumerate(splits, start=1):
            ...
            try:
                state = _fit_state(owner, X[train_idx], y[train_idx], train_df, target, rf_params, rfk_params)
                pred = np.asarray(_predict_state(owner, state, X[test_idx], test_df, target), dtype="float64")
                for local_pos, sample_i in enumerate(test_idx):
                    sample_i = int(sample_i)
                    rec: dict[str, Any] = {}
                    # Preserve original sample columns for audit and optional labels.
                    try:
                        base = df.iloc[sample_i].to_dict()
                    ...
                    rec.update({
                        ...
                        "fold": int(fold),
                        "validation_kind": f"{split_name}-RFK-{label}",
                        "som_value": float(y[sample_i]),
                        "y_true": float(y[sample_i]),
                        "strict_pred_final": float(pred[int(local_pos)]),
                        "pred": float(pred[int(local_pos)]),
                        "x": float(xcoord[sample_i]) if sample_i < len(xcoord) else np.nan,
                        "y": float(ycoord[sample_i]) if sample_i < len(ycoord) else np.nan,
                        "x_32648": float(xcoord[sample_i]) if sample_i < len(xcoord) else np.nan,
                        "y_32648": float(ycoord[sample_i]) if sample_i < len(ycoord) else np.nan,
                        "coord_crs": coord_crs,
                    })
                    for feat in feature_cols or []:
                        if feat in df.columns and feat not in rec:
                            rec[feat] = df.iloc[sample_i][feat]
                    rows.append(rec)
            except Exception as exc:
                _emit_safe(owner, "MODEL", "GCP + AOA 校准折预测跳过", {"fold": int(fold), "error": str(exc)})
                continue
        if not rows:
            return ""
        gcp_dir = Path(out_dir) / "03_GCP输入"
        gcp_dir.mkdir(parents=True, exist_ok=True)
        oof = pd.DataFrame(rows)
        for c in ["som_value", "y_true", "strict_pred_final", "pred", "x", "y", "x_32648", "y_32648"]:
            if c in oof.columns:
                oof[c] = pd.to_numeric(oof[c], errors="coerce")
        if "abs_residual" not in oof.columns:
            oof["abs_residual"] = (oof["som_value"] - oof["strict_pred_final"]).abs()
        # Repeated split aggregation: one calibration record per sample.
        numeric_avg = [c for c in ["som_value", "y_true", "strict_pred_final", "pred", "abs_residual", "x", "y", "x_32648", "y_32648"] + list(feature_cols or []) if c in oof.columns]
        agg = {c: "mean" for c in numeric_avg}
        for c in oof.columns:
            if c not in agg and c != "sample_index":
                agg[c] = "first"
        if "sample_index" in oof.columns:
            oof = oof.groupby("sample_index", as_index=False).agg(agg)
        preferred = [
            "sample_index", "fold", "validation_kind", "coord_crs",
            "x", "y", "x_32648", "y_32648",
            "som_value", "y_true", "strict_pred_final", "pred", "abs_residual",
        ]
        ordered = [c for c in preferred if c in oof.columns] + [c for c in (feature_cols or []) if c in oof.columns and c not in preferred]
        ordered += [c for c in oof.columns if c not in ordered]
        oof = oof[ordered]
        out_csv = gcp_dir / "strict_groupkfold_oof.csv"
        oof.to_csv(out_csv, index=False, encoding="utf-8-sig")
        _emit_safe(owner, "MODEL", "GCP + AOA 校准样点表已生成", {
            "strict_oof_csv": str(out_csv),
            "rows": int(len(oof)),
            "raw_holdout_rows": int(len(rows)),
            "feature_count": int(len(feature_cols or [])),
            "validation": split_name,
        })
        return str(out_csv)
    except Exception as exc:
        _emit_safe(owner, "MODEL", "GCP + AOA 校准样点表生成失败", {"error": str(exc)})
        return ""
...
def run_clean_rfk_model(owner: Any, df: pd.DataFrame, feature_cols: list[str], out_dir: Path, target: dict[str, Any], real_cov_cols: list[str]) -> tuple[Path | None, Path | None]:
    if OrdinaryKriging is None:
        raise RuntimeError("V166 Clean RFK 需要 pykrige。请执行：pip install pykrige")
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    _emit_safe(owner, "MODEL", "V166 Clean RFK 从建模样点表重新开始训练与验证", {"out_dir": str(out_dir)})

    csv_path = getattr(owner, "_model_ready_csv", None)
    if not csv_path or not Path(csv_path).exists():
        raise RuntimeError("V166 Clean RFK 未找到建模样点表.csv，已停止，避免继续沿用内存数据。")
    ...
    _emit_safe(owner, "MODEL", "已识别融合CSV训练诊断特征，开始构建RFK训练矩阵", {
        "rows": int(len(y)),
        "diagnostic_feature_count": int(len(kept_features)),
        "diagnostic_features_head": [str(x) for x in kept_features[:20]],
        "upstream_feature_count": int(len(feature_cols or [])),
        "note": "训练诊断会自动使用建模样点表中的有效数值协变量；正式GeoTIFF仍只使用有匹配预测栅格的特征。"
    })
    ...
    if len(y) < 20:
        raise RuntimeError(f"有效样点数不足，无法执行 V166 Clean RFK：{len(y)}")
    ...
    # : first formal run should not look frozen. Use a lighter default search
    # (24 candidates, 1 spatial tuning split) unless the user overrides env vars.
    tune_splits = int(os.getenv("PRO_CLEAN_RFK_TUNE_SPLITS", "1"))
    _emit_safe(owner, "MODEL", "Clean RFK 开始联合寻参", {
        "candidate_count": int(len(candidates)),
        "tune_splits": int(tune_splits),
        "sample_count": int(len(y)),
        "feature_count": int(X.shape[1]),
        "progress_policy": "每个候选参数开始和结束都会写入日志，避免误判为卡住。",
        "deep_search_hint": "如需更全面寻参，可在.env中设置 PRO_CLEAN_RFK_CANDIDATE_COUNT=128 和 PRO_CLEAN_RFK_TUNE_SPLITS=3。"
    })
    scores = []
    for i, cand in enumerate(candidates, start=1):
        _emit_safe(owner, "MODEL", f"Clean RFK 正在评分候选参数 {i}/{len(candidates)}", {
            "candidate_id": i,
            "rf_params": cand.get("rf_params"),
            "rfk_params": cand.get("rfk_params")
        })
        t0 = time.time()
        sc = _score_candidate(owner, cand, X, y, samples_df, groups, target, n_splits=tune_splits)
        sc["candidate_id"] = i
        sc["elapsed_sec"] = round(time.time() - t0, 3)
        scores.append(sc)
        _emit_safe(owner, "MODEL", f"Clean RFK 候选参数 {i}/{len(candidates)} 完成", {"candidate_id": i, "ok": sc.get("ok"), "rmse": sc.get("rmse"), "r2": sc.get("r2"), "elapsed_sec": sc.get("elapsed_sec"), "error": sc.get("error")})
    ok_scores = [s for s in scores if s.get("ok") and s.get("rmse") is not None]
    if not ok_scores:
        raise RuntimeError("V166 Clean RFK 全部候选参数评分失败，无法建模。")
    ...
    _emit_safe(owner, "MODEL", "Clean RFK 完成联合寻参", {"best_candidate_id": best.get("candidate_id"), "best_rmse": best.get("rmse"), "best_r2": best.get("r2")})
    ...
    _emit_safe(owner, "MODEL", "Clean RFK 开始空间7:3验证", {"test_size": test_size, "sample_count": int(len(y)), "feature_count": int(X.shape[1])})
    ...
    _emit_safe(owner, "MODEL", "Clean RFK 空间7:3验证完成", {"r2": spatial_holdout.get("r2"), "rmse": spatial_holdout.get("rmse"), "test_n": spatial_holdout.get("test_n")})
    _emit_safe(owner, "MODEL", "Clean RFK 开始随机7:3验证", {"test_size": test_size, "sample_count": int(len(y)), "feature_count": int(X.shape[1])})
    ...
    _emit_safe(owner, "MODEL", "Clean RFK 随机7:3验证完成", {"r2": random_holdout.get("r2"), "rmse": random_holdout.get("rmse"), "test_n": random_holdout.get("test_n")})
    ...
    _emit_safe(owner, "MODEL", "Clean RFK 开始重复空间验证", {"splits": int(os.getenv("PRO_V164_REPEATED_SPATIAL_SPLITS", "7"))})
    repeated_spatial = _repeated_holdout(owner, X, y, samples_df, groups, target, best_rf, best_rfk, spatial=True, test_size=test_size, n_splits=int(os.getenv("PRO_V164_REPEATED_SPATIAL_SPLITS", "7")), label="repeated_spatial_aux")
    ...
    _emit_safe(owner, "MODEL", "Clean RFK 重复验证完成", {"spatial_r2": repeated_spatial.get("r2"), "spatial_rmse": repeated_spatial.get("rmse"), "random_r2": repeated_random.get("r2"), "random_rmse": repeated_random.get("rmse")})
    ...
    # V166: formal GeoTIFF deployment is separated from diagnostic training.
    ...
    if mappable_plan.get("mappable_features"):
        try:
            _only_clcd = all(_is_clcd_feature_name(x) for x in (mappable_plan.get("mappable_features") or []))
            if _only_clcd:
                _emit_safe(owner, "MODEL", "提醒：当前正式出图特征主要来自CLCD分类/掩膜", {
                    "mappable_feature_count": mappable_plan.get("mappable_feature_count"),
                    "missing_raster_feature_count": mappable_plan.get("missing_raster_feature_count"),
                    "message": "可以出图，但正式GeoTIFF模型会较弱；CSV全特征仍用于训练诊断，若要高质量制图请补充DEM、气候、植被、土壤属性等对应预测栅格。"
                })
            ...
            if (os.getenv("PRO_V166_TUNE_MAPPABLE_MODEL", os.getenv("PRO_V165_TUNE_MAPPABLE_MODEL", "1")) or "1").strip().lower() not in {"0", "false", "no", "off"}:
                map_candidates = _candidate_space(len(map_y), map_X.shape[1])
                map_scores = []
                for j, cand in enumerate(map_candidates, start=1):
                    _emit_safe(owner, "MODEL", f"可出图特征RFK正在评分候选参数 {j}/{len(map_candidates)}", {"candidate_id": j, "rf_params": cand.get("rf_params"), "rfk_params": cand.get("rfk_params")})
                    t_map0 = time.time()
                    sc2 = _score_candidate(owner, cand, map_X, map_y, map_samples_df, map_groups, target, n_splits=tune_splits)
                    sc2["candidate_id"] = j
                    sc2["elapsed_sec"] = round(time.time() - t_map0, 3)
                    map_scores.append(sc2)
                    _emit_safe(owner, "MODEL", f"可出图特征RFK候选参数 {j}/{len(map_candidates)} 完成", {"candidate_id": j, "rmse": sc2.get("rmse"), "r2": sc2.get("r2"), "elapsed_sec": sc2.get("elapsed_sec"), "error": sc2.get("error")})
            ...
            realtime_splits = int(os.getenv("PRO_REALTIME_METRIC_SPLITS", "5"))
            map_repeated_spatial = _repeated_holdout(owner, map_X, map_y, map_samples_df, map_groups, target, map_best_rf, map_best_rfk, spatial=True, test_size=test_size, n_splits=realtime_splits, label="deployed_realtime_spatial")
            map_repeated_random = _repeated_holdout(owner, map_X, map_y, map_samples_df, None, target, map_best_rf, map_best_rfk, spatial=False, test_size=test_size, n_splits=realtime_splits, label="deployed_realtime_random")
            metric_fingerprint = _runtime_metric_fingerprint(
                model_ready_csv=str(csv_path),
                features=list(map_features or []),
                map_scope=str(target.get("map_scope") or target.get("output_scope") or "full_domain"),
                raster_map=getattr(owner, "_covariate_raster_map", {}) or {},
                row_count=int(len(map_samples_df)),
                target=target,
            )
            map_metrics = {
                "single_spatial_validation": map_spatial,
                "single_random_validation": map_random,
                "realtime_spatial_validation": map_repeated_spatial,
                "realtime_random_validation": map_repeated_random,
                "feature_count": len(map_features),
                "features": map_features,
                "matrix_audit": map_matrix_audit,
                "metric_fingerprint": metric_fingerprint,
                "metric_recalculated_at": time.time(),
                "metric_policy": "V219 realtime: user-facing metrics are recalculated from the current deployable model-ready matrix and current mappable feature set; no diagnostic/full-CSV fallback is allowed.",
            }
            map_matrix_audit.update({"coordinate_crs": map_coord_crs, "spatial_group_count": int(len(np.unique(map_groups))), "spatial_groups_hash": _hash_array(map_groups), "metric_fingerprint": metric_fingerprint})
            (out_dir / "V166_可出图矩阵_审计.json").write_text(json.dumps(map_matrix_audit, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
            target["v166_mapping_matrix_audit"] = {"audit_path": str(out_dir / "V166_可出图矩阵_审计.json"), **map_matrix_audit}
            _emit_safe(owner, "MODEL", "V166已训练正式出图RFK模型", {"mappable_feature_count": len(map_features), "missing_raster_feature_count": mappable_plan.get("missing_raster_feature_count")})
        except Exception as exc:
            mappable_plan["mapping_model_error"] = str(exc)
            map_state = None
            map_features = []
            map_samples_df = None
            _emit_safe(owner, "MODEL", "V166正式出图模型未生成", {"error": str(exc)})
    else:
        _emit_safe(owner, "MODEL", "V166未找到可出图协变量栅格：已完成融合CSV训练验证，但跳过GeoTIFF输出", {"csv_feature_count": mappable_plan.get("csv_feature_count"), "missing_raster_features": mappable_plan.get("missing_raster_features")})
    ...
    matrix_audit.update({
        "version": "V166_clean_rfk_dynamic_features",
        "model_ready_csv": str(csv_path),
        ...
        "validation_design": "V166 computes diagnostic validation on CSV features and, when rasters are incomplete, trains a separate mappable model using only CSV features with matching prediction rasters.",
    ...
    audit_p = out_dir / "V166_训练矩阵_从头审计.json"
    ...
    if map_state is not None and map_features and map_samples_df is not None:
        try:
            strict_oof_csv = _write_formal_gcp_oof(
                owner, out_dir, map_X, map_y, map_samples_df, map_groups, target,
                map_best_rf, map_best_rfk, map_features, label="formal_mappable"
            )
        except Exception as exc:
            _emit_safe(owner, "MODEL", "GCP + AOA 校准样点表生成异常", {"error": str(exc)})
            strict_oof_csv = ""

    # V218 integrity fix:
    # The user-facing accuracy must describe the deployed/formal mapping model,
    # not the diagnostic CSV model. Earlier versions displayed `primary`
    # (diagnostic validation on all usable CSV features) even when the GeoTIFF
    # was generated by a separate mappable-feature model. That made accuracy
    # appear unchanged when the user changed mappable covariates or output scope.
    diagnostic_metrics = {
        "cv": primary.get("cv"),
        "r2": primary.get("r2"),
        ...
        "metric_source": "diagnostic_csv_model_all_usable_features",
    }

    deployment_primary = None
    deployment_aux = None
    if isinstance(map_metrics, dict):
        # V219: user-facing accuracy must be the real-time validation of the
        # deployed mapping matrix. Prefer repeated spatial validation because it
        # is stricter and less dependent on one lucky split. Random validation is
        # kept only as an auxiliary reference.
        deployment_primary = map_metrics.get("realtime_spatial_validation") or map_metrics.get("single_spatial_validation")
        deployment_aux = map_metrics.get("realtime_random_validation") or map_metrics.get("single_random_validation")
    if not deployment_primary:
        deployment_primary = {"cv": "not_available", "r2": None, "rmse": None, "mae": None, "bias": None, "train_n": None, "test_n": None, "split_count": 0, "splits": []}
        deployment_aux = {}
        metric_source = "unavailable_no_deployed_mapping_model"
        metric_warning = "未生成正式出图模型，不能计算本轮最终制图精度；诊断模型指标仅保留后台审计，不在用户界面冒充最终精度。"
    else:
        metric_source = "realtime_deployed_mapping_model_mappable_features"
        metric_warning = None

    metrics = {
        "cv": deployment_primary.get("cv"),
        "r2": deployment_primary.get("r2"),
        "rmse": deployment_primary.get("rmse"),
        "mae": deployment_primary.get("mae"),
        "bias": deployment_primary.get("bias"),
        "train_n": deployment_primary.get("train_n"),
        "test_n": deployment_primary.get("test_n"),
        "split_count": 1,
        "splits": deployment_primary.get("splits"),
        "target_aoi_test_n": deployment_primary.get("target_aoi_test_n"),
        "target_aoi_test_ratio": deployment_primary.get("target_aoi_test_ratio"),
        "target_aoi_r2": deployment_primary.get("target_aoi_r2"),
        "target_aoi_rmse": deployment_primary.get("target_aoi_rmse"),
        "target_aoi_mae": deployment_primary.get("target_aoi_mae"),
        "target_aoi_bias": deployment_primary.get("target_aoi_bias"),
        "target_aoi_warning": deployment_primary.get("target_aoi_warning"),
        "aux_validation": deployment_aux or {},
        "single_spatial_validation": (map_metrics or {}).get("single_spatial_validation") if isinstance(map_metrics, dict) else {},
        "single_random_validation": (map_metrics or {}).get("single_random_validation") if isinstance(map_metrics, dict) else {},
        "realtime_spatial_validation": (map_metrics or {}).get("realtime_spatial_validation") if isinstance(map_metrics, dict) else {},
        "realtime_random_validation": (map_metrics or {}).get("realtime_random_validation") if isinstance(map_metrics, dict) else {},
        "diagnostic_csv_model_validation": diagnostic_metrics,
        "repeated_spatial_validation": repeated_spatial,
        "repeated_random_validation": repeated_random,
        "metric_source": metric_source,
        "metric_warning": metric_warning,
        "deployed_feature_count": int(len(map_features or [])),
        "diagnostic_feature_count": int(len(kept_features or [])),
        "deployed_feature_hash": _stable_hash_text(json.dumps(list(map_features or []), ensure_ascii=False, sort_keys=True, default=str))[:12],
        "diagnostic_feature_hash": _stable_hash_text(json.dumps(list(kept_features or []), ensure_ascii=False, sort_keys=True, default=str))[:12],
        "metric_fingerprint": (map_metrics or {}).get("metric_fingerprint") if isinstance(map_metrics, dict) else None,
        "metric_recalculated_at": (map_metrics or {}).get("metric_recalculated_at") if isinstance(map_metrics, dict) else time.time(),
        "metric_policy": "V219：用户界面主精度每轮从当前正式出图矩阵实时重算；不允许复用旧报告、不允许诊断模型冒充最终精度。",
        "validation_rewrite_note": "V219：用户界面主精度绑定当前正式出图模型的实时重复空间验证；诊断CSV全特征模型指标仅作为后台审计信息保存。",
    }
    if metric_warning:
        _emit_safe(owner, "MODEL", "精度来源警告", {"metric_source": metric_source, "warning": metric_warning})
    _emit_safe(owner, "MODEL", "实时精度已从当前正式出图矩阵重算", {
        "metric_source": metric_source,
        "metric_fingerprint": metrics.get("metric_fingerprint"),
        "deployed_features": list(map_features or []),
        "diagnostic_features": list(kept_features or []),
        "r2": metrics.get("r2"),
    ...
    report: dict[str, Any] = {
        ...
        "rfk_core_version": "V173_clean_cropland_only_clcd_mask",
        "message": "V173模型优先RFK核心：融合CSV动态识别非CLCD特征；CLCD仅用于耕地掩膜，不作为SOM模型特征；正式GeoTIFF只使用非CLCD CSV列与可用预测栅格的交集。",
        "target": target,
        "rows": int(len(y)),
        "feature_count": int(X.shape[1]),
        "feature_cols": kept_features,
        "csv_diagnostic_feature_cols": kept_features,
        "v166_feature_deployment_plan": mappable_plan,
        ...
        "rf_params": map_best_rf if map_state is not None else best_rf,
        "diagnostic_rf_params": best_rf,
        "deployed_rf_params": map_best_rf if map_state is not None else None,
        "metric_source": metrics.get("metric_source"),
        "metric_warning": metrics.get("metric_warning"),
        "diagnostic_csv_model_metrics": diagnostic_metrics,
        "hyperparameter_plan": {
            "mode": "V218_deployed_mapping_model_metric_bound",
            "primary_validation_mode": primary_mode,
            "candidate_count": len(candidates),
            "selected_rf_params": map_best_rf if map_state is not None else best_rf,
            "selected_rfk_params": map_best_rfk if map_state is not None else best_rfk,
            "best_score": map_best if map_state is not None else best,
    ...
    pred_tif = None
    if map_state is not None and map_features and map_samples_df is not None:
        _emit_safe(owner, "MODEL", "开始生成正式GeoTIFF预测图", {"mapping_feature_count": len(map_features), "out_dir": str(out_dir)})
        pred_tif = owner._make_prediction_tif(map_state, map_features, map_samples_df, out_dir, target)
        _emit_safe(owner, "MODEL", "正式GeoTIFF预测图生成完成", {"pred_tif": str(pred_tif) if pred_tif else None})
    else:
        target["prediction_grid_audit"] = {"stopped": True, "reason": "no_mappable_feature_rasters_diagnostic_only", "v166_feature_deployment_plan": mappable_plan}
    report["prediction_grid_csv"] = getattr(owner, "_prediction_grid_csv", None)
    report["strict_oof_csv"] = strict_oof_csv
    report["features"] = list(map_features or kept_features or [])
    report["feature_cols"] = list(map_features or kept_features or [])
    report["outputs"] = {
        ...
        "prediction_grid_csv": getattr(owner, "_prediction_grid_csv", None),
        "prediction_grid_covariates_csv": getattr(owner, "_prediction_grid_csv", None),
        "grid_covariates_csv": getattr(owner, "_prediction_grid_csv", None),
    }
    ...
    try:
        report.setdefault("outputs", {})["report_json"] = str(report_path)
    except Exception:
        pass
    ...
    _emit_safe(owner, "MODEL", "V166 Clean RFK 建模完成", {"report": str(report_path), "pred_tif": str(pred_tif) if pred_tif else None, "metrics": metrics})
